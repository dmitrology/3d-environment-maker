...
import { SafeLoading, SafeError } from "./html-wrapper"
import { withCanvasCheck } from "./canvas-context"
import dynamic from "next/dynamic"

// Dynamically import components that use R3F hooks
const DynamicHybridModel = dynamic(() => import("./hybrid-model").then((mod) => ({ default: mod.HybridModel })), {
  ssr: false,
  loading: () => <SafeLoading message="Loading model component..." />,
})
...
function ModelSpawnerBase({ prompt }: ModelSpawnerProps) {
  ...
  if (loading) {
    return <SafeLoading message="Analyzing prompt..." />
  }

  if (error) {
    return <SafeError message={error.message || "Unknown error"} />
  }

  if (models.length === 0) {
    return <SafeError message="No models to display. Try a different prompt." />
  }
...
export const ModelSpawner = withCanvasCheck(ModelSpawnerBase)
